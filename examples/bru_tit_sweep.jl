...
Results are written to bru_tit_sweep_results.csv for plotting.
"""
...
# ── TIT sweep ────────────────────────────────────────────────────────────────
TIT_range_R = range(1460.0, 2060.0, length=25)

println("TIT_R, TIT_K, W_shaft_kW, W_elec_kW, Q_heater_kW, Q_recup_kW, eta_cycle_pct, T_turb_out_R, T_recup_hot_out_R, PR_turb")
...
for TIT_R in TIT_range_R
    ...
    println("$(round(TIT_R,digits=1)), $(round(R_to_K(TIT_R),digits=1)), " *
            ...
            "$(round(η_cyc,digits=2)), $(round(T_tout,digits=1)), " *
            "$(round(T_rout,digits=1)), $(round(PR_t,digits=4))")
    push!(results, (TIT_R, W_shaft, W_elec, Q_heat, η_cyc))
end

println("\nDesign point (2060 °R):  W_shaft = $(round(results[end][2],digits=2)) kW,  W_elec = $(round(results[end][3],digits=2)) kW,  η = $(round(results[end][5],digits=1)) %")
