...
- **`thermo/`** — `FluidProperties` abstract interface (`cp`, `enthalpy`, `entropy`, `density`, `gamma`, `viscosity`, `conductivity`, `prandtl` + inversions `T_from_h`, `T_from_s`, `h_from_s`). Elements call only this interface, never a concrete backend. Four backends: `NobleGasMixture`/`HeXe(M)` (analytic virial EOS + transport, Tournier/El-Genk AIAA 2006-4154 — the default for He-Xe work), `FPTFluid` (reads NPSS FPT table files — kept for apples-to-apples NPSS comparison), `IdealGasFluid`, `ConstantPropertyLiquid` (coolants).
